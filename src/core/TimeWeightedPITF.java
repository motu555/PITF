 ...
 * 与TAPITF相比，训练时去掉了流行度权重
 * 1.只在预测时添加上从所有训练数据中得到的流行度权重
 * 2.根本不使用流行度权重
 */


public class TimeWeightedPITF extends TagMFRecommender {
    ...
    public TimeWeightedPITF(List<Post> trainPostsParam, List<Post> testPostsParam, int dimParam, double initStdevParam, int iterParam,
