...
public class DenseMatrix implements Serializable {
    ...
    /**
     * Matrix transpose multiplication with a dense matrix
     *
     * @param mat a dense matrix
     * @return a dense matrix with results of matrix multiplication
     */
    public DenseMatrix tMult(DenseMatrix mat) {
        assert this.numRows == mat.numRows;

        DenseMatrix res = new DenseMatrix(this.numColumns, mat.numColumns);
        for (int i = 0; i < res.numRows; i++) {
            for (int j = 0; j < res.numColumns; j++) {

                double product = 0;
                for (int k = 0; k < this.numRows; k++)
                    product += data[k][i] * mat.data[k][j];
    ...
    public DenseVector tMult(DenseVector vec) {
        assert this.numRows == vec.size;

        DenseVector res = new DenseVector(this.numColumns);
        for (int j = 0; j < res.size; j++) {
            double product = 0;
            for (int i = 0; i < this.numRows; i++) {
                product += data[i][j] * vec.data[i];
            }
            res.set(j, product);
        }
     ...
     * Do {@code A + B} matrix operation
     *
     * @return a matrix with results of {@code A = A + B}
     */
    public void addEquals(DenseMatrix mat) {
        assert numRows == mat.numRows;
        assert numColumns == mat.numColumns;
