...
public class DenseVector implements Serializable {
     ...
     * @return a dense vector by scaling a value to all entries of current vector
     */
    public void scaleSelf(double val) {
        for (int i = 0; i < size; i++)
     ...
     * Do vector operation: {@code a + b}
     *
     * @return a dense vector with results of {@code a = a + b}
     */
    public void addSelf(DenseVector vec) {
        assert size == vec.size;
     ...
     * @return a dense vector with results of {@code a = a - b}
     */
    public void minusSelf(DenseVector vec) {
        assert size == vec.size;
    ...
    public DenseVector times(DenseVector vec) {
        assert size == vec.size;
        DenseVector res = new DenseVector(this.size);
        for (int i = 0; i < size; i++) {
    ...
    public DenseVector divide(DenseVector vec) {
        assert size == vec.size;
        DenseVector res = new DenseVector(this.size);
        for (int i = 0; i < size; i++) {
            res.set(i, this.get(i) / vec.get(i));
